...
export default function CreateFieldCommentCard(props) {
  const {
    horizontal,
    style,
  } = props;
  ...
  const handleCreateFieldCommentButton = async () => {
    if (!textAreaValue.length <= 0) {
      ...
      await axios
        .put(`${FIELDS_URL}/add/comment/${props.item.key}`, body)
        .then((r) => {
        ...
        .catch((error) => {
          ...
          setSnackbarMessage("Error al comentar en la cancha");
        ...
        <Block flex row>
          <Block style={styles.cardButtons}>
            <Button
              ...
              onPress={() => handleCreateFieldCommentButton(textAreaValue)}
            >
              Commentar
            </Button>
          </Block>
...
CreateFieldCommentCard.propTypes = {

...
export default class SearchMatches extends React.Component {
  state = {
    results: [],
    search: "",
    active: false
  ...
  handleSearchChange = async search => {
    let results = await (await axios.get(`${MATCHES_URL}?data=${search}`)).data.data
  ...
  renderResult = result => {
    ...
    return (
      ...
      >
        ...
        <MatchCard item={result} horizontal />
      </Animated.View>
    );
  };

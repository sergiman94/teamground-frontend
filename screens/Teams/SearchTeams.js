...
import { FIELDS_URL, TEAMS_URL } from "../../utils/utils.";
...
export default class SearchTeams extends React.Component {
  state = {
    results: [],
    search: "",
    active: false
  ...
  handleSearchChange = async search => {
    console.log('dasdas')
    let results = (await axios.get(`${TEAMS_URL}?data=${search}`)).data.data
  ...
  renderSearch = () => {
    ...
    return (
      <Input
        ...
        placeholder="Que equipo estas buscando ?"
  ...
  renderResult = result => {
    ...
    return (
      ...
      >
        <TeamTableCard {...this.props} item={result} horizontal/>
      </Animated.View>
    );
  };
